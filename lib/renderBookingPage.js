...
<body class="bp-root">
  ...
  <main class="bp-main" data-nofixed="${previewBanner ? 'true' : 'false'}">
    ...
    <section class="bp-section bp-booking" id="bp-booking" hidden>
      ...
      <div class="bp-guests-total-row">
        <div class="bp-guests-block">
        ...
        <div class="bp-total-block">
          <p class="bp-field-label">Total</p>
  ...
  <div class="bp-sticky-bar" id="bp-sticky-bar" hidden>
  ...
  (function () {
    ...
    function renderTicketOptions(ev) {
      ...
      options.forEach(function (opt, i) {
        ...
        card.className = 'bp-ticket-option' + (i === 0 ? ' is-selected' : '');
        card.setAttribute('data-ticket-id', opt.id);
        card.setAttribute('role', 'radio');
        card.setAttribute('aria-checked', i === 0 ? 'true' : 'false');

        var row = document.createElement('div');
        row.className = 'bp-ticket-option-row';
        ...
        // Inclusions are architected the same way as ticketOptions itself:
        // present in the fixture today, undefined for real events until
        // the canonical schema carries per-tier inclusions. Hidden via CSS
        // ([is-selected] only) rather than removed from the DOM, so
        // selecting a ticket "expands" it without a re-render.
        if (Array.isArray(opt.includes) && opt.includes.length > 0) {
          var includesBlock = document.createElement('div');
          includesBlock.className = 'bp-ticket-includes';
          var includesLabel = document.createElement('p');
          includesLabel.className = 'bp-ticket-includes-label';
          includesLabel.textContent = "What's included";
          var includesList = document.createElement('ul');
          includesList.className = 'bp-ticket-includes-list';
          opt.includes.forEach(function (item) {
            var li = document.createElement('li');
            li.textContent = item;
            includesList.appendChild(li);
          });
          includesBlock.appendChild(includesLabel);
          includesBlock.appendChild(includesList);
          card.appendChild(includesBlock);
        }

        card.addEventListener('click', function () { selectTicket(opt.id); });
