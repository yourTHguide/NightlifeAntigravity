 ...
 *
 * UX refinement (approved mock-up): arriving with ?night=<slug> renders
 * straight into the one-page booking form — no "what are you booking?"
 * re-ask, since we already know. A product with exactly one upcoming
 * occurrence skips the date picker entirely and gets a "one night only"
 * framing instead of a single, pointless date button. Ticket type has a
 * real render hook (renderTicketField) but stays hidden today: the
 * canonical feed carries exactly one effectivePrice per occurrence, so
 * there's nothing to choose between yet — this is presentation
 * architecture only, not new data.
 *
...
<body class="bp-root">
  ...
  (function () {
    ...
    // exactly one effectivePrice, so this always returns a single implicit
    // option — renderTicketField uses that to decide there's nothing worth
    // asking the guest to choose between yet.
    function ticketOptionsFor(ev) {
      if (!ev) return [];
      return [{ id: 'implicit', label: 'General Admission', price: ev.effectivePrice }];
    }

    function renderTicketField(ev) {
      ...
      options.forEach(function (opt) {
        ...
        card.className = 'bp-ticket-option';
        var labelEl = document.createElement('span');
        labelEl.className = 'bp-ticket-option-label';
        labelEl.textContent = opt.label;
        var priceEl = document.createElement('span');
        priceEl.className = 'bp-ticket-option-price';
        priceEl.textContent = baht(opt.price);
    ...
    function renderBooking(events) {
      ...
      // Subtitle is architected for a future per-product tagline field —
      // the canonical feed doesn't send one today, so this simply stays
      // hidden rather than inventing copy.
      ...
      if (sorted.length === 1) {
        ...
        sorted.forEach(function (e) {
          ...
          dotEl.className = 'bp-date-card-dot';
