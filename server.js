...
//  Secure endpoint for VIP/Private experience inquiries
// ═══════════════════════════════════════════════════
app.post('/api/vip-inquiry', async (req, res) => {
    try {
        ...
        // 4. Sanitize event_date and handle flexible date strings safely to prevent Postgres crashes
        let parsedDate = null;
        let flexibleText = '';
        ...
        // 5. Record details in experience_inquiries table
        ...
        console.log(`📋 Experience inquiry recorded: ${inquiry.id}`);

        // 5. Send Instant Email Alert via standard Gmail SMTP (reuse global transporter)
...
app.post('/api/contact', async (req, res) => {
    try {
        ...
        let guestId;
        const tagToAppend = '✉️ Contact Us';
        ...
        if (existingGuest) {
            ...
            console.log(`👤 Guest ${guestId} updated with Contact Us tag`);
        } else {
            const firstName = name.trim().split(/\s+/)[0] || 'Guest';
            const cleanFirstName = firstName.toLowerCase().replace(/[^a-zA-Z0-9]/g, '') || 'guest';
            const phoneDigits = String(normalizedPhone || '').replace(/\D/g, '');
            ...
            guestId = newGuest.id;
            console.log(`👤 New guest ${guestId} created with Contact Us tag`);
        }
        ...
        const { data: inquiry, error: inquiryError } = await supabase
            ...
            .insert({
                ...
                group_size: null,
                occasion: null,
                preferred_vibe: message || 'No message provided',
                budget_range: null
            })
            .select('id')
            .single();
        ...
        console.log(`📋 Contact inquiry recorded: ${inquiry.id}`);

        // 5. Send Rich Email Alert to admin via Gmail SMTP (reuse transporter)
